"""
DDIM 生成：加载 checkpoint，用 DDIMProcess + SimpleUNet 采样并保存图片。
可与 configs/ddim.yaml 配合，或直接传参。
"""
import os
import argparse
...
def load_ddim_checkpoint(
    checkpoint_path: str,
    device: str = None,
    use_ema: bool = True,
):
    """加载 DDIM checkpoint，返回 (model, config_dict)。"""
    if device is None:
...
def sample_ddim(
    ...
    """用 DDIM 从 checkpoint 生成图片并保存到 output_dir。"""
    if seed is not None:
        torch.manual_seed(seed)
    if device is None:
        device = "cuda" if torch.cuda.is_available() else "cpu"

    model, cfg = load_ddim_checkpoint(checkpoint_path, device=device, use_ema=use_ema)
    ...
    if output_dir is None:
        output_dir = get_save_dir("ddim", "samples/generated")
    ...
    with torch.no_grad():
        for i in range(num_batches):
            ...
            samples = diffusion.sample_loop(
                model, shape=shape, num_steps=num_steps, eta=eta
            )
...
def main():
    ...
    parser.add_argument("--num_steps", type=int, default=None, help="DDIM 采样步数")
    parser.add_argument("--eta", type=float, default=None, help="0=确定性, 1=带随机")

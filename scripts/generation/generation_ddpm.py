"""
DDPM 生成：加载 checkpoint，用 DDPMProcess + SimpleUNet 采样并保存图片。
可与 configs/ddpm.yaml 配合，或直接传参。
"""
import os
import argparse
...
def load_ddpm_checkpoint(
    ...
    """
    加载 DDPM checkpoint，返回 (model, config_dict)。
    model 已在 device 上且 eval。
    """
    if device is None:
...
def sample_ddpm(
    ...
    """
    用 DDPM 从 checkpoint 生成图片并保存到 output_dir。
    - output_dir: 为空则用 output/ddpm/samples/generated
    - img_size / batch_size: 为空则从 checkpoint 的 config 里取
    - seed: 固定随机种子
    """
    if seed is not None:
    ...
    model, cfg = load_ddpm_checkpoint(checkpoint_path, device=device, use_ema=use_ema)
    ...
    if output_dir is None:
        output_dir = get_save_dir("ddpm", "samples/generated")
    ...
    with torch.no_grad():
        for i in range(num_batches):
            ...
            samples = diffusion.sample_loop(model, shape=shape)
